...
{
    internal readonly Dictionary<(TAction Action, int Profile), ActionState> States = [];
    ...
    public ActionState this[TAction action, int profile = 0] =>
        States.TryGetValue((action, profile), out var state)
            ? state
            : throw new InvalidOperationException($"No binding exists for action '{action}' at profile {profile}. Call Bind()/BindAxis2D() for it first.");

    /// <summary>Same as the indexer, without throwing when the action was never bound.</summary>
    public bool TryGet(TAction action, out ActionState state, int profile = 0) => States.TryGetValue((action, profile), out state);
}
