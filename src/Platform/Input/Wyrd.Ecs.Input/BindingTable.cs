...
public sealed partial class BindingTable<TAction> where TAction : struct, Enum
{
    ...
    private readonly Dictionary<(ProfileId Profile, TAction Action), Kind> _kinds = [];
    private readonly Dictionary<(ProfileId Profile, TAction Action), HashSet<SDL.Scancode>> _keys = [];
    private readonly Dictionary<(ProfileId Profile, TAction Action), HashSet<MouseButton>> _mouseButtons = [];
    private readonly Dictionary<(ProfileId Profile, TAction Action), (SDL.Scancode Up, SDL.Scancode Down, SDL.Scancode Left, SDL.Scancode Right)> _axes = [];
    private readonly Dictionary<ProfileId, HashSet<DeviceId>> _assignedDevices = [];
    ...
    public BindingTable<TAction> Bind(TAction action, params SDL.Scancode[] keys) => Bind(default, action, keys);
    ...
    public BindingTable<TAction> Bind(ProfileId profile, TAction action, params SDL.Scancode[] keys)
    {
    ...
    public BindingTable<TAction> Bind(TAction action, params MouseButton[] buttons) => Bind(default, action, buttons);
    ...
    public BindingTable<TAction> Bind(ProfileId profile, TAction action, params MouseButton[] buttons)
    {
        ClaimDigital(profile, action);
    ...
    public BindingTable<TAction> BindAxis2D(TAction action, SDL.Scancode up, SDL.Scancode down, SDL.Scancode left, SDL.Scancode right) =>
        BindAxis2D(default, action, up, down, left, right);
    ...
    public BindingTable<TAction> BindAxis2D(ProfileId profile, TAction action, SDL.Scancode up, SDL.Scancode down, SDL.Scancode left, SDL.Scancode right)
    {
        if (_kinds.TryGetValue((profile, action), out var kind) && kind == Kind.Digital)
            throw new InvalidOperationException(
                $"Action '{action}' at profile {profile.Value} is already bound as a digital action. Call Unbind() first to switch it to an axis.");
        _kinds[(profile, action)] = Kind.Axis2D;
        _axes[(profile, action)] = (up, down, left, right);
        return this;
    }

    private void ClaimDigital(ProfileId profile, TAction action)
    {
        if (_kinds.TryGetValue((profile, action), out var kind) && kind == Kind.Axis2D)
            throw new InvalidOperationException(
                $"Action '{action}' at profile {profile.Value} is already bound as an axis. Call Unbind() first to switch it to a digital action.");
    ...
    public BindingTable<TAction> Unbind(TAction action) => Unbind(default, action);

    /// <summary>Same as <see cref="Unbind(TAction)"/>, targeting a specific <paramref name="profile"/> instead of profile 0.</summary>
    public BindingTable<TAction> Unbind(ProfileId profile, TAction action)
    {
    ...
    public BindingTable<TAction> Unbind(TAction action, SDL.Scancode key) => Unbind(default, action, key);
    ...
    public BindingTable<TAction> Unbind(ProfileId profile, TAction action, SDL.Scancode key)
    {
    ...
    public BindingTable<TAction> Unbind(TAction action, MouseButton button) => Unbind(default, action, button);
    ...
    public BindingTable<TAction> Unbind(ProfileId profile, TAction action, MouseButton button)
    {
    ...
    public BindingTable<TAction> AssignDevice(ProfileId profile, DeviceId deviceId)
    {
    ...
    public BindingTable<TAction> UnassignDevice(ProfileId profile)
    {
    ...
    internal void UnassignDeviceById(DeviceId deviceId)
    {
    ...
    internal IEnumerable<(ProfileId Profile, TAction Action, Kind Kind)> BoundActions() =>
    ...
    internal IReadOnlySet<SDL.Scancode> KeysFor(ProfileId profile, TAction action) =>
    ...
    internal IReadOnlySet<MouseButton> MouseButtonsFor(ProfileId profile, TAction action) =>
    ...
    internal (SDL.Scancode Up, SDL.Scancode Down, SDL.Scancode Left, SDL.Scancode Right)? AxisFor(ProfileId profile, TAction action) =>
    ...
    internal IReadOnlySet<DeviceId>? AssignedDevicesFor(ProfileId profile) =>
