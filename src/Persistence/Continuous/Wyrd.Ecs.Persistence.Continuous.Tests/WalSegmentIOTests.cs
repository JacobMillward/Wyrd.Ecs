...
public class WalSegmentIOTests
{
    ...
    public void WriteRecord_ThenTryReadRecord_RoundTripsAllFields()
    {
        ...
        WalSegmentIO.TryReadRecord(stream, out var kind, out var tick, out var readEntityId, out var discriminator, out var schemaHash, out var payload).Should().BeTrue();
    ...
    public void WriteRecord_WithNoSchemaHash_RoundTripsAsNull()
    {
        ...
        WalSegmentIO.TryReadRecord(stream, out _, out _, out _, out _, out var schemaHash, out _).Should().BeTrue();
    ...
    public void WriteRecord_ForEveryRecordKind_RoundTripsTheKind()
    {
        foreach (var kind in Enum.GetValues<WalRecordKind>())
        {
            ...
            WalSegmentIO.TryReadRecord(stream, out var readKind, out _, out _, out _, out _, out _).Should().BeTrue();
    ...
    public void WriteRecord_CalledMultipleTimes_EachRecordReadsBackInOrder()
    {
        ...
        WalSegmentIO.TryReadRecord(stream, out var firstKind, out var firstTick, out var readFirst, out var firstDiscriminator, out _, out _).Should().BeTrue();
        ...
        WalSegmentIO.TryReadRecord(stream, out var secondKind, out var secondTick, out var readSecond, out var secondDiscriminator, out _, out _).Should().BeTrue();
    ...
    public void TryReadRecord_OnAnEmptyStream_ReturnsFalse()
    {
        ...
        WalSegmentIO.TryReadRecord(stream, out _, out _, out _, out _, out _, out _).Should().BeFalse();
    }
    ...
    public void TryReadRecord_OnAStreamTruncatedMidRecord_ReturnsFalseWithoutThrowing()
    {
        ...
        var act = () => WalSegmentIO.TryReadRecord(truncatedStream, out _, out _, out _, out _, out _, out _);
        act.Should().NotThrow();

        using var truncatedStreamAgain = new MemoryStream(truncatedBytes);
        WalSegmentIO.TryReadRecord(truncatedStreamAgain, out _, out _, out _, out _, out _, out _).Should().BeFalse();
    }
    ...
    public void TryReadRecord_OnARecordWithACorruptedByte_ReturnsFalse()
    {
        ...
        WalSegmentIO.TryReadRecord(corruptedStream, out _, out _, out _, out _, out _, out _).Should().BeFalse();
    }
