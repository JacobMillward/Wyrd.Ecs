...
public class WorldDirtyReadTests
{
    ...
    public void ReadChanges_WithALaterWatermark_SkipsEarlierEntries()
    {
        var world = new World();
        using var tracking = world.TrackChanges<Position>();
        Entity entity = world.Commands.CreateEntity(new Position()); // tick 1
        world.ApplyCommands();
    ...
    public void ReadChanges_SpansMultipleArchetypes()
    {
        ...
        Entity onlyPosition = world.Commands.CreateEntity(new Position());
        Entity withVelocity = world.Commands.CreateEntity(new Position(), new Velocity());
        world.ApplyCommands();
    ...
    public void ReadChanges_AtDifferentWatermarks_BothSeeTheRowsCurrentTickAndValue()
    {
        ...
        slowSeenTicks.Should().Equal([4], "there is one current tick-stamp per row, not a log of every past touch, so both watermarks see the same single entry at the row's latest tick");
        fastSeenTicks.Should().Equal([4], "there is one current tick-stamp per row, not a log of every past touch, so both watermarks see the same single entry at the row's latest tick");
    }
