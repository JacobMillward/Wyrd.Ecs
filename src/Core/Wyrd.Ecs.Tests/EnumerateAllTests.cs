...
public class EnumerateAllTests
{
    ...
    public void EnumerateAll_YieldsOneEntryPerRegisteredComponentOnEveryEntity()
    {
        ...
        Entity onlyPosition = world.Commands.CreateEntity(new Position { X = 1f });
        Entity both = world.Commands.CreateEntity(new Position { X = 2f }, new Velocity { X = 3f });
        world.ApplyCommands();
    ...
    public void EnumerateAll_ThenDeserializingEachResult_ReconstructsEquivalentEntities()
    {
        ...
        Entity rebuilt = target.Commands.CreateEntity();
