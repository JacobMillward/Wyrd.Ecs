...
/// <summary>Serializes a <typeparamref name="T"/> component's current value to bytes. Registered against a stable discriminator via <see cref="CodecRegistry.Register{T}"/>, never against <see cref="Internal.TypeIndex{T}"/>, which is not stable across a process restart.</summary>
