...
/// other query surface -- <see cref="World.Query{TAccess0}(ChunkAction{TAccess0})"/> and
/// (eventually) generator-emitted unbounded-shape queries alike -- is built on this one, so
/// archetype resolution and chunk access are implemented in exactly one place.
...
public sealed class ArchetypeQuery
{
    ...
    private ArchetypeQuery(QueryFilter filter) => _filter = filter;
