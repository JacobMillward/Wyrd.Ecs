...
/// <summary>Serializes a <typeparamref name="T"/> relation edge's payload to bytes. Registered against a stable discriminator via <see cref="CodecRegistry.RegisterRelation{T}"/>, never against <see cref="Internal.TypeIndex{T}"/>, which is not stable across a process restart.</summary>
