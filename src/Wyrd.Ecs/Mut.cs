...
/// component slot, when this component type currently has at least one registered
/// consumer; otherwise indexing never marks anything, since nothing would ever read
/// it. This is the tracked mutation path; there is no separate untracked accessor.
/// Marking is tick-stamped and deduplicated: an entity touched many times in one tick
/// appends exactly one entry to the change log, not many. See the design's
...
public readonly ref struct Mut<T> : IComponentAccessor<Mut<T>> where T : struct, IComponent
{
    ...
    private Mut(Span<T> items, Span<int> lastMarkedTick, int tick, DirtyLog dirtyLog, int start, bool tracked)
    {
        _items = items;
    ...
    /// <inheritdoc/>
    public static Mut<T> CreateChunk(Array items, int[] lastMarkedTick, int tick, DirtyLog dirtyLog, int start, int length, bool tracked) =>
        new(((T[])items).AsSpan(start, length), lastMarkedTick.AsSpan(start, length), tick, dirtyLog, start, tracked);
    ...
    /// <typeparamref name="T"/> component. Marking is skipped entirely when nobody is
    /// currently registered to consume this component type's changes.
    /// </summary>
