...
A `BindingTable<TAction>` supports more than one player through `profile`, an integer everywhere its methods default to `0`. Two players sharing a keyboard by key range (WASD vs. arrow keys) need nothing more than two `BindAxis2D` calls at different profiles, see [Input](/engine/input/). Two players each on their own physical keyboard need one more step: telling each profile which device is actually theirs.

## Why profiles need device assignment

```csharp
var bindings = new BindingTable<PlayerAction>()
    .BindAxis2D(profile: 0, PlayerAction.Move, SDL.Scancode.W, SDL.Scancode.S, SDL.Scancode.A, SDL.Scancode.D)
    .BindAxis2D(profile: 1, PlayerAction.Move, SDL.Scancode.W, SDL.Scancode.S, SDL.Scancode.A, SDL.Scancode.D);
```
...
```csharp
bindings.AssignDevice(profile: 0, deviceId: firstKeyboardId)
        .AssignDevice(profile: 1, deviceId: secondKeyboardId);
```
...
```csharp
Input.TryGet(PlayerAction.Move, out var player1Move, profile: 0);
Input.TryGet(PlayerAction.Move, out var player2Move, profile: 1);
```
